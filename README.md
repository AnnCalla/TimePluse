...
Patrix 是一个基于 Wails、Go、React 和 TypeScript 的桌面应用，集成计时、四象限计划和 Markdown 便签。
...
隐藏到后台后，Patrix 会显示在 Windows 通知区域（托盘）中。右键托盘图标可选择“显示主窗口”“迷你番茄钟”或“彻底退出”。
...
- 窗口标题文字：修改 `main.go` 中的 `Title: "Patrix"`。
- 应用内左上角标题：修改 `frontend/src/Layout.tsx` 中的 `Patrix` 文本。
...
- Windows 构建产物为 `build/bin/Patrix.exe`。
